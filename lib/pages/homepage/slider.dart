...
class _ReferralCarouselState extends State<ReferralCarousel> {
  ...
  final List<Map<String, dynamic>> setA = [
    ...
    },
    {
      ...
      "title": "thankYouSlips",
      "image": "assets/images/nirmal.png",
    },
    {
      "name": "MATHIARASU M",
  ...
  Future<void> loadChapterAndFetch() async {
    const storage = FlutterSecureStorage();
    final storedChapterId = await storage.read(key: 'chapter_id');

    setState(() {
      currentChapterId = storedChapterId;
    });
  ...
  Future<void> fetchTopPerformers() async {
    ...
    if (response.isSuccess && response.data != null) {
      ...
      // If API fails or returns nothing, fallback to static sets
      setState(() {
        referrals = currentChapterId == specialChapterId ? setA : setB;
        _isLoading = false;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (referrals.isEmpty) {
      // Final fallback in case everything failed
      referrals = currentChapterId == specialChapterId ? setA : setB;
    }
