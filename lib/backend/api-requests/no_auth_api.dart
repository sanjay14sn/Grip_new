  ...
  static Future<ApiResponse> makeRequest({
    ...
    try {
      ...
      switch (method.toUpperCase()) {
        case 'POST':
          response = await http.post(uri, headers: _defaultHeaders(headers), body: body != null ? jsonEncode(body) : null);
          break;
        case 'GET':
          response = await http.get(uri, headers: _defaultHeaders(headers));
          break;
        case 'PUT':
          response = await http.put(uri, headers: _defaultHeaders(headers), body: body != null ? jsonEncode(body) : null);
          break;
        case 'PATCH':
          response = await http.patch(uri, headers: _defaultHeaders(headers), body: body != null ? jsonEncode(body) : null);
          break;
        case 'DELETE':
          response = await http.delete(uri, headers: _defaultHeaders(headers), body: body != null ? jsonEncode(body) : null);
          break;
        default:
          return ApiResponse(statusCode: 405, isSuccess: false, message: 'Unsupported HTTP method: $method', data: null);
      }

      return _processResponse(response);
    } catch (e) {
      return ApiResponse(statusCode: 500, isSuccess: false, message: 'Error: $e', data: null);
    }
  }
